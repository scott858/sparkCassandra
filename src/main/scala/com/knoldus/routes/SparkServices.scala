...
trait SparkService extends DatabaseAccess {
  ...
  val sparkRoutes: Route = {
    ...
    } ~ path("retrieve" / "experiment_id" / Segment / "mac_address" / Segment / "time_bucket" / Segment) {
      (experimentId: String, macAddress: String, timeBucket: String) =>
        get {
          complete {
            try {
              val idAsRDD: Option[Array[UuidPtp1588TimingPacket]] = retrievePTP(experimentId, macAddress, timeBucket)
              ...
              case ex: Throwable =>
                logger.error(ex, ex.getMessage)
                HttpResponse(StatusCodes.InternalServerError, entity = s"Error found for ids : $experimentId")
            }
          }
        }
    }
